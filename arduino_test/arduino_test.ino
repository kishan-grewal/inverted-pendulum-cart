...
// Sprint trajectory parameters
const float SPRINT_DISTANCE_M = 2.01f;
const float SPRINT_A_MAX      = 0.32f;   // [m/s^2]  gives ~5 s over 2 m
const float SPRINT_V_MAX      = 1.0f;    // [m/s]    cap on cruise speed
...
void setup() {
  ...
  while (digitalRead(START_BUTTON_PIN) == HIGH) {
    ...
    while (digitalRead(CONTROL_SELECT_BUTTON_PIN) == LOW) {
      if ((micros() - start_time_us > 200000) && !control_mode_changed) {
        ...
        switch (control_mode) {
          case 0: Serial.println("LQR");  break;
          case 1: Serial.println("POLE"); break;
  ...
  // Pre-compute sprint timings for trapezoidal/triangular profile
  {
    ...
    if (2.0f * d_accel >= D) {
      // Triangular profile: peak speed is limited by distance, no cruise
      t_accel = sqrtf(D / a);
    ...
    Serial.print("Sprint profile: t_accel="); Serial.print(sprint_t_accel);
    Serial.print(" t_cruise=");               Serial.print(sprint_t_cruise);
    Serial.print(" t_total=");                Serial.println(sprint_t_total);
  }
...
void loop() {
  ...
  // Pause: 20ms press stops loop. Resume: 200ms hold to continue.
  ...
  pendulum_encoder_angle = angle_rad * (180.0f / 3.14159265f);
  ...
  if (task_mode == 1) {
    // ----------------------------------------------------------------
    // Sprint: proper trapezoidal position + velocity reference
    // ----------------------------------------------------------------
    ...
    const float a     = SPRINT_A_MAX;
    const float ta    = sprint_t_accel;
    const float tc    = sprint_t_cruise;
    const float vm    = a * ta;  // actual peak velocity from profile

    float pos_ref_local  = 0.0f;  // relative to sprint_x_start, unsigned
    float vel_ref_signed = 0.0f;

    if (t >= sprint_t_total) {
      // ---- Finished: hold final position ----
      pos_ref_local  = D_abs;
      vel_ref_signed = 0.0f;
      sprint_active    = false;
      ...
      // ---- Accel phase ----
      vel_ref_signed = a * t;
      pos_ref_local  = 0.5f * a * t * t;

    } else if (t < ta + tc) {
      // ---- Cruise phase ----
      float t2       = t - ta;
      vel_ref_signed = vm;
      pos_ref_local  = (0.5f * a * ta * ta) + vm * t2;

    } else {
      // ---- Decel phase ----
      float t3           = t - (ta + tc);
      float pos_end_accel  = 0.5f * a * ta * ta;
      float pos_end_cruise = pos_end_accel + vm * tc;
      vel_ref_signed     = vm - a * t3;
      pos_ref_local      = pos_end_cruise + vm * t3 - 0.5f * a * t3 * t3;
    }

    x_ref    = sprint_x_start + dir * pos_ref_local;
    xdot_ref = dir * vel_ref_signed;

    theta_ref     = 0.0f;
    theta_dot_ref = 0.0f;

  } else {
    // ---- Stabilisation or Recovery: hold origin ----
    x_ref         = 0.0f;
    xdot_ref      = 0.0f;
    theta_ref     = 0.0f;
    theta_dot_ref = 0.0f;
  ...
  // LQR
  float state[4]  = { estimated_position, estimated_velocity,
                      kalman.getTheta(),  kalman.getThetaVelocity() };
  ...
  // Serial (teleplot, throttled 10ms)
  ...
  if (current_time - last_print >= 10000) {
    ...
    Serial.println(">xdot_ref:"           + String(xdot_ref,            3));

...
def load_csv(csv_path: str) -> Tuple[List[float], List[float], List[float], List[float]]:
    ...
    with open(csv_path, "r", newline="", encoding="utf-8") as f:
        ...
        required = {"t_sec", "pendulum_angle_deg", "estimated_velocity", "kalman_x"}
    ...
    return t_list, angle_list, velocity_list, position_list
...
def plot(
    ...
    ax3.plot(t, position, linewidth=1.2, color="C2")
    ax3.set_title("Cart Position vs Time")
    ...
    if save_dir:
        ...
        fig3.savefig(os.path.join(save_dir, "cart_position_vs_time.png"), dpi=200)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot pendulum angle, cart velocity, and cart position from a recorded CSV."
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(pendulum_angle_vs_time.png, cart_velocity_vs_time.png, cart_position_vs_time.png)."
        ),
    )
    ...
    t, angles, v, x = load_csv(csv_path)
    plot(t, angles, v, x, save_dir=args.save_dir)
